...
- **跟读评测**：长文自动按约 24 秒分段录音，逐段识别后合并全文，再统一计算发音、流利度和完整度
...
- **辩题库**：当前内置 386 条辩题，另含独立的“思辨中国”辩题区域
...
- 音频流程：`wx.cloud.uploadFile` → 云函数下载临时文件 → 百度语音逐段识别 → 合并全文评分 → 删除临时文件。每段控制在 1MB 以内，避免百度接口和云函数请求体的单条音频限制。
