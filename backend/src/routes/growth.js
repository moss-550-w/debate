...
const { normalizeRole, canManageStudent } = require('../middleware/rbac');
...
function buildSpeechData(records) {
  ...
  return {
    ...
    history: ordered.slice(0, 10).map(record => ({ _id: record._id, type: record.type || 'speech', score: scoreOf(record), created_at: record.created_at })),
    stats: { totalCount: ordered.length, totalDuration, avgScore: average(ordered, 'overall'), daysActive: activeDays.size },
  };
}
...
function debateActivities(portfolios, turns) {
  const portfolioActivities = portfolios.map(record => {
    ...
    return { _id: record._id, type: 'portfolio', title: record.title || record.topic_title || '作品集记录', created_at: record.created_at, dimensions: result.dimensions, overall: result.overall };
  });
  ...
  return [...portfolioActivities, ...turnActivities].filter(activity => activity.created_at).sort((left, right) => String(left.created_at).localeCompare(String(right.created_at)));
}

function averageDimension(activities, key) {
  const values = activities.map(activity => activity.dimensions[key]).filter(value => value !== null && value !== undefined);
  return values.length ? Math.round(values.reduce((sum, value) => sum + value, 0) / values.length) : 0;
}

function overallScore(dimensions, activities) {
  const weighted = DEBATE_DIMENSIONS.reduce((sum, item) => sum + (dimensions[item.key] ? dimensions[item.key] * item.weight : 0), 0);
...
function buildDebateData(portfolios, turns) {
  ...
  const dimensions = {};
  DEBATE_DIMENSIONS.forEach(({ key }) => { dimensions[key] = averageDimension(activities, key); });
  ...
  const recommendations = [...DEBATE_DIMENSIONS].sort((left, right) => (dimensions[left.key] || 0) - (dimensions[right.key] || 0)).slice(0, 2).map(item => ({
    dimension: item.key,
    title: `加强${item.label}`,
  ...
  return {
    ...
    recommendations,
    history: activities.slice(-10).reverse().map(activity => ({ _id: activity._id, type: activity.type, title: activity.title, score: activity.overall || 0, created_at: activity.created_at })),
  };
}
...
router.get('/:userId', authMiddleware, async (req, res) => {
  try {
    ...
    const targetUser = isSelf ? null : await authStore.findUserById(userId);
    const isManagementRole = ['developer', 'teacher'].includes(normalizeRole(req.user.role));
    if (!isSelf && (!isManagementRole || !targetUser || !canManageStudent(req.user, targetUser))) {
      return res.status(403).json({ code: 403, message: 'Forbidden', data: null });
    }
    const ids = [...new Set((isSelf ? [req.user.userId, req.user.openid] : [targetUser._id, targetUser.openid]).filter(Boolean))];
    const [speechGroups, portfolioGroups, debateGroups] = await Promise.all([
      Promise.all(ids.map(id => practiceStore.listByUser(id))),
      Promise.all(ids.map(id => portfolioStore.listByUser(id))),
      Promise.all(ids.map(id => debateStore.listByUser(id))),
    ]);
    ...
    return res.json({ code: 200, message: 'ok', data: { baseline: speech.baseline, latest: speech.latest, history: speech.history, stats: speech.stats, speech, debate } });
  } catch (err) {
